...
/* ============================ 相机与场景常量 ============================ */
/* 把散布在各处的"魔法数字"集中命名，方便统一调整。 */
#define CAM_DIST_MIN  5.0f   // 相机到目标的最近距离（滚轮缩放下限）
#define CAM_DIST_MAX  120.0f // 相机到目标的最远距离（滚轮缩放上限）
#define CAM_PITCH_MAX 1.5f   // 相机俯仰角上限（弧度，≈86°，避免翻到地面以下）
#define GRID_LINES    20     // 地面网格等分份数（20×20 格子）
#define BLINK_PERIOD  0.5f   // 闪烁灯亮/灭切换周期（秒）
#define TERM_PERIOD   0.5f   // 播放时终端打印状态的时间间隔（秒）
#define FORM_HEIGHT   5.0f   // 编队队形统一飞行高度（Z 轴，米）
#define PATH_SAMPLES  48     // 编辑时平滑路径预览的采样段数
#define COLOR_COUNT   8      // 预设灯光颜色数量（LC / LCN 数组长度）
#define DEMO_COUNT    12     // 一键示例表演生成的无人机数量
#define UI_MSG_TIME   2.5f   // 状态消息在屏幕顶部停留的时间（秒）
